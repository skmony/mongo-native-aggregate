...
router.get('/:id', (req, res, next) => {
  db.getDb()
    .db()
    .collection('products')
    .findOne(new ObjectId(req.params.id)).then(productDoc => {
      productDoc.price = productDoc.price.toString();
...
router.patch('/:id', (req, res, next) => {
  ...
  db.getDb()
    .db()
    .collection('products')
    .updateOne({ _id: new ObjectId(req.params.id) }, { $set: updatedProduct }).then(productDoc => {
...
router.delete('/:id', (req, res, next) => {
  db.getDb()
    .db()
    .collection('products')
    .deleteOne({ _id: new ObjectId(req.params.id) }).then(productDoc => {
      res.status(200).json({ message: 'Product Deleted', productId: productDoc.id });
